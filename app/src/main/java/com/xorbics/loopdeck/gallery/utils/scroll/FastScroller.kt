package com.xorbics.loopdeck.gallery.utils.scroll
...
class FastScroller(
    ...
    init {
        ...
        try {
            ...
            mAutoHideDelay = typedArray.getInteger(
                R.styleable.FastScrollRecyclerView_fastScrollAutoHideDelay,
                DEFAULT_AUTO_HIDE_DELAY
            )
            mThumbInactiveState = typedArray.getBoolean(
                R.styleable.FastScrollRecyclerView_fastScrollThumbInactiveColor,
                false
            )
            mThumbActiveColor = typedArray.getColor(
                R.styleable.FastScrollRecyclerView_fastScrollThumbColor,
                0x79000000
            )

            val trackColor = typedArray.getColor(
                R.styleable.FastScrollRecyclerView_fastScrollTrackColor,
                0x28000000
            )
            val popupBgColor = typedArray.getColor(
                R.styleable.FastScrollRecyclerView_fastScrollPopupBgColor,
                -0x1000000
            )
            val popupTextColor = typedArray.getColor(
                R.styleable.FastScrollRecyclerView_fastScrollPopupTextColor,
                -0x1
            )
            val popupTextSize = typedArray.getDimensionPixelSize(
                R.styleable.FastScrollRecyclerView_fastScrollPopupTextSize,
                Utils.toScreenPixels(resources, 44f)
            )
            val popupBackgroundSize = typedArray.getDimensionPixelSize(
                R.styleable.FastScrollRecyclerView_fastScrollPopupBackgroundSize,
                Utils.toPixels(resources, 88f)
            )
            val popupPosition = typedArray.getInteger(
                R.styleable.FastScrollRecyclerView_fastScrollPopupPosition,
        ...
        mHideRunnable = Runnable {
            if (!isDragging) {
                ...
                mAutoHideAnimator = ObjectAnimator.ofInt(
                    this@FastScroller,
                    "offsetX",
                    (if (Utils.isRtl(mRecyclerView.resources)) -1 else 1) * width
                )
    ...
    fun handleTouchEvent(
        ev: MotionEvent, downX: Int, downY: Int, lastY: Int,
        ...
        when (action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (isDragging) {
                    ...
                    val sectionName =
                        mRecyclerView.scrollToPositionAtProgress((boundedY - top) / (bottom - top))
                    mPopup.setSectionName(sectionName)
                    mPopup.animateVisibility(!sectionName.isEmpty())
                    mRecyclerView.invalidate(
                        mPopup.updateFastScrollerBounds(
                            mRecyclerView,
                            mThumbPosition.y
                        )
                    )
    ...
    fun draw(canvas: Canvas) {
        ...
        canvas.drawRect(
            (mThumbPosition.x + mOffset.x).toFloat(),
            mOffset.y.toFloat(),
            (mThumbPosition.x + mOffset.x + width).toFloat(),
            (mRecyclerView!!.height + mOffset.y).toFloat(),
            mTrack
        )
        ...
        canvas.drawRect(
            (mThumbPosition.x + mOffset.x).toFloat(),
            (mThumbPosition.y + mOffset.y).toFloat(),
            (mThumbPosition.x + mOffset.x + width).toFloat(),
            (mThumbPosition.y + mOffset.y + thumbHeight).toFloat(),
            mThumb
        )
    ...
    private fun isNearPoint(x: Int, y: Int): Boolean {
        mTmpRect.set(
            mThumbPosition.x,
            mThumbPosition.y,
            mThumbPosition.x + width,
            mThumbPosition.y + thumbHeight
        )
    ...
    fun setThumbPosition(x: Int, y: Int) {
        ...
        mInvalidateRect.set(
            ...
            mOffset.y,
            mThumbPosition.x + mOffset.x + width,
            mRecyclerView!!.height + mOffset.y
        )
        mThumbPosition.set(x, y)
        mInvalidateTmpRect.set(
            mThumbPosition.x + mOffset.x,
            mOffset.y,
            mThumbPosition.x + mOffset.x + width,
            mRecyclerView.height + mOffset.y
