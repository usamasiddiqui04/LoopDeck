...
import com.example.loopdeck.gallery.utils.scroll.FastScrollRecyclerView
import com.example.loopdeck.gallery.utils.scroll.FastScroller
import com.example.loopdeck.gallery.utils.scroll.Utils
...
class FastScrollPopup(
    ...
    fun draw(canvas: Canvas) {
        if (isVisible) {
            ...
            canvas.drawText(
                mSectionName!!, ((mBgBounds.width() - mTextBounds.width()) / 2).toFloat(),
    ...
    fun updateFastScrollerBounds(recyclerView: FastScrollRecyclerView, thumbOffsetY: Int): Rect {
        ...
        if (isVisible) {
            ...
            val bgPadding =
                Math.round(((mBackgroundSize - mTextBounds.height()) / 10).toFloat()) * 5
            ...
            if (popupPosition == FastScroller.FastScrollerPopupPosition.CENTER) {
                ...
                mBgBounds.top = Math.max(
                    edgePadding,
                    Math.min(mBgBounds.top, recyclerView.height - edgePadding - bgHeight)
                )
            }
